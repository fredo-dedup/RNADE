...
type Bernstein5 <: NADEDistribution
  w::Vector{Float64}
  dw::Matrix{Float64}
...
length(d::Bernstein5) = d.N

function update!(d::Bernstein5, uw::Vector{Float64}, x₀=0.)
  d.x₀ = x₀
...
#
# i = 1
# show(rdiff(ex, uw=Vector{Float64}, evalmod=Try))
# quote
#     _tmp1 = abs(uw)
#     _tmp2 = sum(_tmp1)
#     _tmp3 = size(_tmp1)
...
### Bernstein5 polynomials Distribution
...
function logpdf(d::Bernstein5, x::Float64)
  # x = 0.35 ; d = pd ; x₀ = 0.2
  pm = fp(x, d.w, d.binom)
  -log(pm)
end
...
# x, x₀ = 0.5, 0.
function dlogpdf!(d::Bernstein5, x::Float64, duw::Vector{Float64})
...
function dtest(uw, x, x₀, index)
  ...
  d0 = Bernstein5(length(uw))
  update!(d0, uw, x₀)

  uw1 = copy(uw)
  setindex!(uw1, w0+δ, index)
  d1 = Bernstein5(length(uw1))
  update!(d1, uw1, x₀)
