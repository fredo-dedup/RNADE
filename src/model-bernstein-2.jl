...
#   with normalization w = 1 / (1 + exp(-x))  / sum(..)
#########################################################################
...
type Bernstein2 <: NADEDistribution
  w::Vector{Float64}
  dw::Matrix{Float64}
...
length(d::Bernstein2) = d.N

function update!(d::Bernstein2, uw::Vector{Float64}, x₀=0.)
  d.x₀ = x₀
  ...
  copy!(ew, ew ./ (ewp1 .* ewp1 * sw2))  # mem reuse
  for i in 1:length(uw)
    for j in 1:length(uw)
      d.dw[i,j] = ew[i] * ( Float64(i==j) - w2[j] / sw2 )
    end
  end
...
### Bernstein2 polynomials Distribution
...
function logpdf(d::Bernstein2, x::Float64)
  # x = 0.35 ; d = pd ; x₀ = 0.2
  pm = fp(x, d.w, d.binom)
  -log(pm)
end
...
# x, x₀ = 0.5, 0.
function dlogpdf!(d::Bernstein2, x::Float64, duw::Vector{Float64})
...
function dtest(uw, x, x₀, index)
  ...
  d0 = Bernstein2(length(uw))
  update!(d0, uw, x₀)

  uw1 = copy(uw)
  setindex!(uw1, w0+δ, index)
  d1 = Bernstein2(length(uw1))
  update!(d1, uw1, x₀)
