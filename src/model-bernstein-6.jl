...
#  With dirac density on 0.
#########################################################################

# w[1:end-1] for weights of Bernstein polynomial
# w[end] for weight on 0.

using Distributions

import .RNADE: length, rand, logpdf, dlogpdf!, update!

type Bernstein6 <: NADEDistribution
  w::Vector{Float64}
  dw::Matrix{Float64}
...
function Bernstein6(n::Int)
  N = n
  binom = Float64[binomial(N-2,i) for i in 0:N-2]
  w = Array(Float64, N)
  dw = Array(Float64, N, N)
  Bernstein6(w, dw, binom, N, 0.)
end

length(d::Bernstein6) = d.N

function update!(d::Bernstein6, uw::Vector{Float64}, x₀=0.)
  d.x₀ = x₀
...
# pick a value
function rand(d::Bernstein6)
  ci = rand(Categorical(d.w))
  ci == d.N && return 0.
  ticdf[(d.N-2, ci-1)][rand(1:101)]
end

# uw = [0., -1, 1, 2, 1, 2]
# d = Bernstein6(length(uw)) ; update!(d, uw)
...
function logpdf(d::Bernstein6, x::Float64)
  x <= pwidth && return -log(d.w[end] / pwidth)
...
# x, x₀ = 0.5, 0.
function dlogpdf!(d::Bernstein6, x::Float64, duw::Vector{Float64})
  if x <= pwidth
    pm = d.w[end] / pwidth
    fill!(duw, 0.)
    duw[end] = 1. / pwidth
...
function dtest(uw, x, x₀, index)
  ...
  d0 = Bernstein6(length(uw))
  update!(d0, uw, x₀)

  uw1 = copy(uw)
  setindex!(uw1, w0+δ, index)
  d1 = Bernstein6(length(uw1))
  update!(d1, uw1, x₀)
