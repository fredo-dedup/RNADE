################################################################
#
#  setup 17 using Bernstein polynomials
#  activation sigmoide
#  avec pénalisation
#
################################################################
...
module Model
  ...
  export loglik, dloglik!

  #### elementary model
  N = 10
  w = rand(Normal(), N)
  cw = Float64[binomial(N-1,i) for i in 0:N-1]
  ...
  @eval fp(x::Float64, w::Vector{Float64}, cw::Vector{Float64}) = $ep  # prob function
  fp(0.5, w, cw)

  @eval fl(x::Float64, w::Vector{Float64}, cw::Vector{Float64}) = log($ep)  # ll function
  fl(0.5, w, cw)

  edl = rdiff(:(log($ep)), allorders=false, w=Vector{Float64}, ignore=[:x])
  @eval fdl(x::Float64, w::Vector{Float64}, cw::Vector{Float64}) = $edl # diffs of loglik function
  fdl(0.5, w, cw)
  ...
  const pwidth = 1e-2
  function loglik(x::Float64, epars::Vector{Float64}, cw::Vector{Float64}, x₀::Float64)
    # epars = weights bernstein distrib + 3 weights for 0, x0 and 1.
    epars = rand(Normal(),6)

    pm = fl(x, epars[1:end-3], cw)  # bernstein part
    nw = log(1. .+ exp(epars))
    nwx =
