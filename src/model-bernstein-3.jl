...
type Bernstein3 <: NADEDistribution
  w::Vector{Float64}
  dw::Matrix{Float64}
...
length(d::Bernstein3) = d.N

function update!(d::Bernstein3, uw::Vector{Float64}, x₀=0.)
  d.x₀ = x₀
...
### Bernstein3 polynomials Distribution
...
# dist = Bernstein3(5)
# dps = zeros(dist.w)
# dfp!(0.2, [1., 0., 0., 0., 0.], dist.binom, dps) ; dps
# [ [ fp(0.2, [1., 0., 0., 0., 0.] + eye(5), dist.binom) ]
# [ fp() ]
...
function logpdf(d::Bernstein3, x::Float64)
  # x = 0.35 ; d = pd ; x₀ = 0.2
  pm = fp(x, d.w, d.binom)
  -log(pm)
end
...
# x, x₀ = 0.5, 0.
function dlogpdf!(d::Bernstein3, x::Float64, duw::Vector{Float64})
...
function dtest(uw, x, x₀, index)
  ...
  d0 = Bernstein3(length(uw))
  update!(d0, uw, x₀)

  uw1 = copy(uw)
  setindex!(uw1, w0+δ, index)
  d1 = Bernstein3(length(uw1))
  update!(d1, uw1, x₀)
